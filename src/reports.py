    ...
    def send_report(self, user_id, reported):
        ...
        try:
            with con.cursor() as cur:
                ...
                cur.execute('SELECT COUNT(*) FROM Reports WHERE Reported = %s AND UserId = %s', (str(reported), str(user_id)))
    ...
    def send_love(self, user_id, loved):
        ...
        try:
            with con.cursor() as cur:
                ...
                num_reportes = int(cur.fetchone()[0])
                cur.execute('SELECT COUNT(*) FROM Reports WHERE Reported = %s AND UserId = %s', (str(loved), str(user_id)))
                already_reported = bool(cur.fetchone()[0])
                if num_reportes == n_reportes:  # Si ya tiene 5 reportes
