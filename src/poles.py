...
class Poles:
    def pole_handler(self, message):
        ...
        try:
            cur = con.cursor()
            if user_id in poles:
                cur.execute('UPDATE Ranking SET Points = Points - 3 WHERE UserId = ?', (user_id,))
                ...
                cur.execute('UPDATE Ranking SET Points = Points - 1 WHERE UserId = ?', (user_id,))
    ...
    def subpole_handler(self, message):
        ...
        try:
            ...
            if user_id in poles:
                ...
                cur.execute('UPDATE Ranking SET Points = Points + 2 WHERE UserId = ?', (user_id,))
                if is_saturday:
    ...
    def send_nuke(self, message):
        ...
        try:
            cur = con.cursor()
            cur.execute('SELECT Points FROM Ranking WHERE UserId = ?', (user_id,))
            user_points = cur.fetchone()[0]
            if user_points >= nuke:
                cur.execute('UPDATE Ranking SET Points = Points - ? WHERE UserId = ?', (nuke, user_id))
    ...
    def send_perros(self, message):
        ...
        try:
            ...
            if user_points >= perros:
                cur.execute('UPDATE Ranking SET Points = Points - ? WHERE UserId = ?', (perros, user_id))
    ...
    def change_group_name_bot(self, message):
        ...
        try:
            cur = con.cursor()
            cur.execute('UPDATE GroupNames SET groupName = ?  WHERE position = ?', (name, position))
